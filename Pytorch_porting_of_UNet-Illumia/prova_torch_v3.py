...
class NetCDFDataset(Dataset):
    def __init__(self, features_file_path, leadtime=1, start_year=2015, end_year=2019):
...
def main():
    ...
    dataset = NetCDFDataset(features_path, leadtime=1, start_year=2015, end_year=2017)
    ...
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler, pin_memory=True, num_workers=4)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=1, sampler=val_sampler, pin_memory=True, num_workers=4)
    ...
    if rank == 0:
        ...
        # Visualization
        # Since num_workers=4, it's safe to iterate over the dataloader(which gives  back batches)
        # is a method that creates a new tensor that shares storage with the original tensor but does not require gradients. 
